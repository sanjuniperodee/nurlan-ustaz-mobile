...
EdgeInsets getEdgeInsets(int length,List<EventsType?> events,EventsType event){
  switch(length){
    case (1):
      return const EdgeInsets.only(
          left:  2, right: 2.0, top: 1.0,bottom: 1);
    case (2):
      return  EdgeInsets.only(
          left: events.toList().indexOf(event) == 0 ?  2 : 8, right: 2.0, top: 1.0,bottom: 1);
    case (3):
      return EdgeInsets.only(
  left: events.toList().indexOf(event) == 0? 2 :
  events.toList().indexOf(event) == 1 ? 8  : 14, right: 2.0, top: 1.0,bottom: 1);
    default:
      return EdgeInsets.only();
  }
}
