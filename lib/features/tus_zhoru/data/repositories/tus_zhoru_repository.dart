...
class TusZhoruRepositoryImpl extends TusZhoruRepository {
  ...
  @override
  Future<Either<Failure, List<TusZhoruDTO>>> customTusZhoru({String? search, int? page, bool? isFirstCall}) async {
    if (await networkInfo.isConnected) {
      try {
        final List<TusZhoruDTO> customTusZhoruList = await remoteDS.customTusZhoru(
            search: search,
            currentPage: page,
            isFirstCall: isFirstCall);
  ...
  @override
  Future<Either<Failure, TusZhoruDTO>> createTusZhoru({required String title, required String description}) async {
    if (await networkInfo.isConnected) {
      try {
        final TusZhoruDTO result = await remoteDS.createTusZhoru(title: title,description: description);
  ...
  @override
  Future<Either<Failure, FreedomPaymentDTO>> createCustomTusZhoruPayment({required int id, required String userIp, required String backUrl}) async {
    if (await networkInfo.isConnected) {
      try {
        final FreedomPaymentDTO result = await remoteDS.createCustomTusZhoruPayment(id: id,userIp: userIp,backUrl: backUrl);
  ...
  @override
  Future<Either<Failure, FreedomPaymentDTO>> createTusZhoruPayment({required int id, required String userIp, required String backUrl}) async {
    if (await networkInfo.isConnected) {
      try {
        final FreedomPaymentDTO result = await remoteDS.createTusZhoruPayment(id: id,userIp: userIp,backUrl: backUrl);
  ...
  @override
  Future<Either<Failure, TusZhoruDTO>> getTusZhoruById({required int id}) async {
    if (await networkInfo.isConnected) {
      try {
  ...
  @override
  Future<Either<Failure, TusZhoruDTO>> getCustomTusZhoru({required int id}) async {
    if (await networkInfo.isConnected) {
      try {
